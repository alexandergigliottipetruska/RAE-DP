...
def load_configs():
    ...
    # CRITICAL: This was missing in your version!
...
def start_swarm(cfg, target_nodes=None):
    ...
    remote_cmd = (
        ...
        f"nohup python3 {cfg['worker_script']} > {cfg['log_directory']}/worker_$(hostname).log 2>&1 &"
    )
...
def check_status(cfg):
    domain = cfg.get('domain', '')
    password = cfg.get('ssh_password')
    worker_script = cfg.get('worker_script', 'swarm_worker.py')
    ...
    for node in cfg['nodes']:
        # This command checks if the python process is running 
        # AND pulls GPU memory usage if the machine is up.
        check_cmd = (
        ...
        # We use wait=True (default) here because we need the output
        result = run_remote(cfg, node, domain, check_cmd, password, is_start_cmd=False)
    
        # 255 is the standard exit code for an SSH connection failure/timeout
...
def restart_idle(cfg):
    ...
    # Filter for nodes that are explicitly "IDLE"
...
if __name__ == "__main__":
    # Use the new merged loader
    config = load_configs()
    
    if len(sys.argv) < 2:
        print("Usage: python3 swarm_manager.py [start|stop|status|restart_idle]")
