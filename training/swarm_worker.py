...
def load_swarm_configs():
    base_path = os.path.join("configs", "swarm_config.yaml")
...
def upload_to_hf_and_clean(trial_dir, trial_number):
    ...
    api = HfApi()
    # Pull the repo ID from secrets/config
    repo_id = SWARM_CFG["project"].get("hf_repo_id", "Denass04/RAEDiTRobotics-stage1-sweeps") 

    study_name = SWARM_CFG["project"]["study_name"]
    
    # Nested folder structure: StudyName / Trial_X
...
def sample_from_config(trial, search_space):
    ...
    for name, config in search_space.items():
        ...
        # 3. Call Optuna using the UPDATED kwargs (Not config["kwargs"]!)
...
# 1. Setup scratch space dynamically from config
    ...
    # 4. Unpack everything directly into the Stage1Config dataclass!
    config = Stage1Config(
    ...
    try:
        ...
        # SUCCESS: We survived 15 epochs! Upload to HF and clean up.
        upload_to_hf_and_clean(trial_dir, trial.number)
        return best_rec_loss
        ...
        if os.path.exists(trial_dir):
            shutil.rmtree(trial_dir)
        raise e #Let Optuna catch the error so it does not squew metrics
